...
namespace Cuyahoga.Core.Infrastructure.Transactions
{
	...
	public class TransactionalFileWriter : IResource
	{
		...
		public void CopyDirectory(string sourceDirectory, string targetDirectory)
		{
            ...
            this._directoriesToCopy.Add(new CopyLocations(sourceDirectory, targetDirectory));
		}
		...
		{
            // Delete directories from temp location
            foreach (CopyLocations directoryLocations in this._directoriesToCopy)
            {
                if (Directory.Exists(directoryLocations.SourceLocation))
                {
                    Directory.Delete(directoryLocations.SourceLocation, true);
                }
            }
		...
		public void Commit()
		{
            //Move directories from temp to permanent location
            foreach (CopyLocations directoryLocations in this._directoriesToCopy)
            {
                Directory.Move(directoryLocations.SourceLocation, directoryLocations.TargetLocation);
            }

            //// Deleted Directories
            //foreach (CopyLocations directoryLocations in this._deletedDirectories)
            //{
            //    Directory.Delete(directoryLocations.SourceLocation, true);
            //}
		...
		{
            // Remove temp files for _directoriesToCopy
            foreach (CopyLocations directoryLocations in this._directoriesToCopy)
            {
                Directory.Delete(directoryLocations.SourceLocation, true);
            }
