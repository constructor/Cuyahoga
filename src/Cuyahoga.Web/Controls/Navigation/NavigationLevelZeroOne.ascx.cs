			...
			try
			{
				if (this.Page is PageEngine)
				{
					...
					if (this._page.CuyahogaUser != null)
					{
                        ...
                        bool isAdmin = this._page.CuyahogaUser.IsInRole("Administrator");//HasPermission(AccessLevel.Administrator);
                        this.liAdmin.Visible = isAdmin;
						this.hplAdmin.NavigateUrl = this._page.ResolveUrl("~/Admin");
						this.hplAdmin.Visible = isAdmin;
					}
				}
			}
